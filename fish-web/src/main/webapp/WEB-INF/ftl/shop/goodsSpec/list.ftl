	...
	<div class="ibox float-e-margins">
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/shop/goodsSpec/list/${goodsId}">商品规格(${goodsName})</a></li>
                </ul>
            </div>
        </div>
	    <div class="ibox-content" style="border-color: transparent">
	        ...
	        <div class="example-wrap">
	            <div class="example">
				<@shiro.hasPermission name="shop:goodsSpec:add">
                    <button type="button" class="btn btn-outline btn-primary" onclick="add()">创建</button>
				</@shiro.hasPermission>
				<@shiro.hasPermission name="shop:goodsSpec:delete">
		                ...
		                {{# for(var i=0;i<d.length;i++){  }}
							<tr class="long-td">
		                        ...
		                        <td>{{d[i].keyName}}</td>
							  	<td>{{d[i].price}}</td>
                                <td>
								<@shiro.hasPermission name="shop:goodsSpec:edit">
                                    <a href="${basePath}/shop/goodsSpec/edit/${goodsId}/{{d[i].id}}"  class="btn btn-primary btn-xs">
...
<div class="spiner-example">
	...
	function Ajaxpage(curr){
		...
		$.ajax({
			url: "${basePath}/shop/goodsSpec/list",
	...
	function add(){
	    window.location.href="${basePath}/shop/goodsSpec/add/${goodsId}";
	...
	function DelAll(){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                url: "${basePath}/shop/goodsSpec/delete",
                data:"id="+checkArray,
                type: "POST",
                dataType:'json',
