        ...
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/shop/goodsOptionalType/list">选项属性</a></li>
                </ul>
            </div>
        </div>
	    <div class="ibox-content" style="border-color: transparent">
	        ...
	        <div class="row">
	            <div class="col-sm-12">  
	                ...
	                <form id="myForm" name="admin_list_sea" class="form-search" method="post" action="${basePath}/shop/brand/list">
	        ...
	        <div class="example-wrap">
	            <div class="example">
				<@shiro.hasPermission name="shop:goodsOptionalType:add">
                    <button type="button" class="btn btn-outline btn-primary" onclick="add()">创建</button>
				</@shiro.hasPermission>
				<@shiro.hasPermission name="shop:goodsOptionalType:delete">
		            ...
		            <table class="table table-bordered table-hover">
		                ...
		                {{# for(var i=0;i<d.length;i++){  }}
							<tr class="long-td">
                                <td>
                                    <input type="checkbox" class="i-checks" value="{{d[i].typeId}}">
                                </td>
		                        <td>{{d[i].typeId}}</td>
							  	<td>{{d[i].name}}</td>
                                <td>
								<@shiro.hasPermission name="shop:goodsOptionalType:edit">
                                    <a href="${basePath}/shop/goodsOptionalType/edit/{{d[i].typeId}}"  class="btn btn-primary btn-xs">
...
<div class="spiner-example">
	...
	function Ajaxpage(curr){
		...
		$.ajax({
			url: "${basePath}/shop/goodsOptionalType/list",
	...
	function add(){
	    window.location.href="${basePath}/shop/goodsOptionalType/add";

	}
    function edit(id){
        window.location.href="${basePath}/shop/goodsOptionalType/edit/"+id;
	...
	function DelAll(){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                url: "${basePath}/shop/goodsOptionalType/delete",
                data:"id="+checkArray,
                type: "POST",
                dataType:'json',
