	...
	<div class="ibox float-e-margins">
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/shop/goodsCateAttr/list/${id}">分类属性:${id}</a></li>
                </ul>
            </div>
        </div>
	    <div class="ibox-content" style="border-color: transparent">
	        ...
	        <div class="example-wrap">
	            <div class="example">
				<@shiro.hasPermission name="shop:goodsCateAttr:add">
                    <button type="button" class="btn btn-outline btn-primary" onclick="add(${id})">创建</button>
				</@shiro.hasPermission>
				<@shiro.hasPermission name="shop:goodsCateAttr:delete">
                    <button type="button" class="btn btn-outline btn-primary" onclick="DelAll()">批量删除</button>
				</@shiro.hasPermission>
                    <a href="${basePath}/shop/goodsCate/list" style="float: right;"  class="btn btn-white">返回</a>
		            <table class="table table-bordered table-hover">
		                <thead>
                                ...
                                <th style="text-align:center;">作为筛选</th>
		                ...
		                {{# for(var i=0;i<d.length;i++){  }}
							<tr class="long-td">
                                <td>
                                    <input type="checkbox" class="i-checks" value="{{d[i].attrId}}">
                                </td>
		                        <td>{{d[i].attrId}}</td>
							  	<td>{{d[i].name}}</td>
                                <td>
								{{# if(d[i].opts!=null){ }}
								{{#
								var opts=JSON.parse(d[i].opts);
								for(var k=0;k<opts.length;k++){  }}
								<button type="button" class="mr10 mbtn btn">{{opts[k]}}</button>
								{{# } }}
								{{# }else{ }}
								未设置
								{{# } }}
								...
								<@shiro.hasPermission name="shop:goodsCateAttr:edit">
                                    <a href="${basePath}/shop/goodsCateAttr/edit/{{d[i].attrId}}"  class="btn btn-primary btn-xs">
...
<div class="spiner-example">
	...
	function Ajaxpage(curr){
		...
		$.ajax({
			url: "${basePath}/shop/goodsCateAttr/list/"+id,
			data:{page:curr||1},
			type: "POST",
			dataType:'json',
	...
	function add(id){
	    window.location.href="${basePath}/shop/goodsCateAttr/add/"+id;

	}
    function edit(id){
        window.location.href="${basePath}/shop/goodsCateAttr/edit/"+id;
	...
	function DelAll(){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                url: "${basePath}/shop/goodsCateAttr/delete",
                data:"id="+checkArray,
                type: "POST",
                dataType:'json',
