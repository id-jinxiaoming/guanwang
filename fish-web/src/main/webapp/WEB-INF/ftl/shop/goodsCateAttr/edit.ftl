        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form class="form-horizontal m-t" id="myForm" action="${basePath}/shop/goodsCateAttr/edit" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-3">
                                <input id="opts" type="text"  value="${item.opts}" class="form-control" name="opts" required="" placeholder="" aria-required="true"><span>多个中间用,号分隔</span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-3 control-label">计量单位：</label>
                            <div class="col-sm-3">
                                <input id="uom" type="text"  value="${item.uom}" class="form-control" name="uom" required="" aria-required="true"><span>例如"英寸"、"像素"、"Kg"等等，如该属性不使用计量单位，请留空</span>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <div class="radio i-checks">
                                    <label><input type="radio" value="1" name="filtrate" <#if item.filtrate == 1>checked</#if> > <i></i> 是</label>
                                    <label><input type="radio" value="0" name="filtrate" <#if item.filtrate == 0>checked</#if>> <i></i> 否</label>
                                </div>
                            </div>
                        ...
                        <div class="form-group">
                            <div class="col-sm-8 col-sm-offset-3">
                                <input type="hidden" name="attrId" value="${item.attrId}" />
                                <button class="btn btn-primary" type="submit">提交</button>
                                <a  class="btn btn-danger" type="button"  href="${basePath}/shop/goodsCateAttr/list/${item.cateId}"><i class="fa fa-close"></i> 返回</a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                success: function(data) { // data 保存提交后返回的数据，一般为 json 数据
                    ...
                    if(data.status == 200){
                        ...
                        layer.msg(data.message,{icon: 6,time:2000,shade: 0.1},function(index){
                            window.location.href="${basePath}/shop/goodsCateAttr/list/${item.cateId}";
                        });
                    }else{
