        ...
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/shop/brand/list">品牌管理</a></li>
                </ul>
            </div>
        </div>
	                ...
	                <form id="myForm" name="admin_list_sea" class="form-search" method="post" action="${basePath}/shop/brand/list">
	                    <div class="col-sm-3" style="padding-left: 0px;padding-right: 0px;">
	                        <div class="input-group">
	                            <input type="text" id="key" class="form-control" name="key"  placeholder="输入需查询的品牌名" />
				...
				<@shiro.hasPermission name="shop:brand:delete">
		            ...
		            <table class="table table-bordered table-hover">
		                ...
		                {{# for(var i=0;i<d.length;i++){  }}
							<tr class="long-td">
                                <td>
                                    <input type="checkbox" class="i-checks" value="{{d[i].brandId}}">
                                </td>
		                        <td>{{d[i].brandId}}</td>
							  	<td>{{d[i].brandName}}</td>
                                <td>
                                    <img src="{{d[i].brandLogo}}" class="img-circle" style="width:30px;height:30px" onerror="this.src='${basePath}/static/webupload/head_default.gif'"/>
                                </td>
                                <td>{{d[i].seq}}</td>
                                <td>

								<@shiro.hasPermission name="shop:brand:edit">
                                    <a href="${basePath}/shop/brand/edit/{{d[i].brandId}}"  class="btn btn-primary btn-xs">
...
<div class="spiner-example">
	...
	function add(){
	    window.location.href="${basePath}/shop/brand/add";

	}
    function edit(id){
        window.location.href="${basePath}/shop/brand/edit/"+id;
	...
	function DelAll(){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                url: "${basePath}/shop/brand/delete",
                data:"id="+checkArray,
                type: "POST",
                dataType:'json',
