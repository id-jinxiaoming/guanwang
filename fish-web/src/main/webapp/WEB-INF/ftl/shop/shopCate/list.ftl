	...
	<div class="ibox float-e-margins">
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/shop/shopCate/list">商家分类</a></li>
                </ul>
            </div>
        </div>
	    <div class="ibox-content" style="border-color: transparent">
	        ...
	        <div class="example-wrap">
	            <div class="example">
				<@shiro.hasPermission name="shop:shopCate:add">
                    <button type="button" class="btn btn-outline btn-primary" onclick="add()">创建</button>
				</@shiro.hasPermission>
				<@shiro.hasPermission name="shop:shopCate:delete">
                    <button type="button" class="btn btn-outline btn-primary" onclick="DelAll()">批量删除</button>
				</@shiro.hasPermission>
                    <a href="${basePath}/shop/shopCate/list" style="float: right;"  class="btn btn-white">返回</a>
		            <table class="table table-bordered table-hover">
		                ...
		                {{# for(var i=0;i<d.length;i++){  }}
							<tr class="long-td">
                                <td>
                                    <input type="checkbox" class="i-checks" value="{{d[i].cateId}}">
                                </td>
		                        <td>{{d[i].cateId}}</td>
                                <td>

                                    <img src="{{d[i].imgUrl}}" style="width:80px;height:80px" />
								...
								<@shiro.hasPermission name="shop:shopCate:edit">
                                    <a href="${basePath}/shop/shopCate/edit/{{d[i].cateId}}"  class="btn btn-primary btn-xs">
...
<div class="spiner-example">
	...
	function Ajaxpage(curr){
		...
		$.ajax({
			url: "${basePath}/shop/shopCate/list",
			data:{page:curr||1},
			type: "POST",
			dataType:'json',
	...
	function add(){
	    window.location.href="${basePath}/shop/shopCate/add";

	}
    function edit(){
        window.location.href="${basePath}/shop/shopCate/edit";
	...
	function DelAll(){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                url: "${basePath}/shop/shopCate/delete",
                data:"id="+checkArray,
                type: "POST",
                dataType:'json',
