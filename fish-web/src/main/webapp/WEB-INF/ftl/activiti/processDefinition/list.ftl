	            ...
	            <div class="example">
		            ...
		            <table class="table table-bordered table-hover">
		                <thead>
                                ...
                                <th style="text-align:center;">diagramResourceName</th>
		                ...
		                {{# for(var i=0;i<d.length;i++){  }}
							<tr class="long-td">
                                ...
                                <td>{{d[i].diagramResourceName}}</td>
                                <td>{{d[i].resourceName}}</td>
                                <td>{{d[i].tenantId}}</td>
                                <td>{{d[i].version}}</td>
                                <td>{{d[i].category}}</td>
                                <td>
								<@shiro.hasPermission name="activiti:processDefinition:show">
                                    <button type="button" class="btn btn-primary  btn-sm" onclick="show('{{d[i].deploymentId}}','{{d[i].diagramResourceName}}')">查看</button>
								</@shiro.hasPermission>
                                </td>
...
<div class="spiner-example">
	...
	function Ajaxpage(curr){
		$.ajax({
			url: "${basePath}/activiti/processDefinition/list",
			data:{page:curr||1},
			type: "POST",
			dataType:'json',
	...
	function show(deploymentId,diagramResourceName){
        layer.open({
            type: 2,
            title: '流程图',
            area: ['600px', '380px'],
            ...
            closeBtn: 1,
            shadeClose: true,
            content: '${basePath}/activiti/processDefinition/show?deploymentId='+deploymentId+'&diagramResourceName='+diagramResourceName
        });
