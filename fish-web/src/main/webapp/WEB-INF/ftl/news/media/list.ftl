        ...
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/news/media/list">新闻媒体列表</a></li>
                </ul>
            </div>
        </div>
        <div class="ibox-content" style="border-color: transparent">
            ...
            <div class="row">
                <div class="col-sm-12">
                    ...
                    <form id="myForm" name="admin_list_sea" class="form-search" method="post" action="${basePath}/news/media/list">
                        <div class="col-sm-3" style="padding-right: 0px;padding-left: 0px;">
                            <div class="input-group">
                                <input type="text" id="key" class="form-control" name="key"  placeholder="输入需查询的标题" />
            ...
            <div class="example-wrap">
                <div class="example">
				<@shiro.hasPermission name="news:media:add">
                    <button type="button" class="btn btn-outline btn-primary" onclick="add()">创建</button>
				</@shiro.hasPermission>
				<@shiro.hasPermission name="news:media:delete">
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <script id="datalist" type="text/html">
                            ...
                            <tr class="long-td">
                                ...
                                <td>
                                    {{# if(d[i].type==1){ }}
                                    <span style="background-color: #dd9222;color: #ffffff;border-radius: 5px;font-size: 18px;padding: 5px; ">行业资讯</span>
                                    {{# }else{ }}
                                    <span style="background-color: #00ee00;color: #ffffff;border-radius: 5px;font-size: 18px;padding: 5px; ">公司新闻</span>
                                    {{# } }}
                                </td>
                                <td>{{d[i].title}}</td>
                                <td >
                                    {{# if(d[i].isRoofPlacement==1){ }}
                                    <span style="background-color: #1cc09f;color: #ffffff;border-radius: 5px;font-size: 18px;padding: 5px; ">置顶</span>
                                    {{# }else{ }}
                                    <span style="background-color: #0d8ddb;color: #ffffff;border-radius: 5px;font-size: 18px;padding: 5px; ">正常</span>
                                    {{# } }}
                                </td>
                                <td>
                                ...
                                <td>
								<@shiro.hasPermission name="news:media:edit">
                                    <a href="${basePath}/news/media/edit/{{d[i].id}}"  class="btn btn-primary  btn-xs">
                                        <i class="fa fa-paste"></i> 编辑</a>
								</@shiro.hasPermission>
                               <@shiro.hasPermission name="news:media:check">

                                    <button type="button" class="btn btn-primary  btn-xs" onclick="check('{{d[i].id}}')"><i class="fa fa-paste"></i>置顶操作</button>
                               </@shiro.hasPermission>
...
<script type="text/javascript">
    ...
    function add(){
        window.location.href="${basePath}/news/media/add";

    }
    function edit(id){
        window.location.href="${basePath}/news/media/edit/"+id;
    ...
    function DelAll(){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                url: "${basePath}/news/media/delete",
                data:"id="+checkArray,
                type: "POST",
                dataType:'json',
    ...
    function check(id){
        ...
        indexWin = layer.open({
            ...
            area: ['500px', '350px'],
            title:"审核",
            content:"${basePath}/news/media/check/"+id
