        ...
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/users/usersGroup/list">会员组</a></li>
                </ul>
            </div>
        </div>
	    <div class="ibox-content" style="border-color: transparent">
	        ...
	        <div class="row">
	            <div class="col-sm-12">  
	                ...
	                <form name="admin_list_sea" class="form-search" method="get" action="${basePath}/users/usersGroup/list">
	        ...
	        <div class="example-wrap">
	            <div class="example">
				<@shiro.hasPermission name="users:usersGroup:add">
                    <button type="button" class="btn btn-outline btn-primary" onclick="add()">创建</button>
				</@shiro.hasPermission>
				<@shiro.hasPermission name="users:usersGroup:delete">
		            ...
		            <table class="table table-bordered table-hover">
		                ...
		                {{# for(var i=0;i<d.length;i++){  }}
							<tr class="long-td">
                                <td>
                                    <input type="checkbox" class="i-checks" value="{{d[i].groupId}}">
                                </td>
                                <td>{{d[i].groupId}}</td>
                                <td>{{d[i].groupName}}</td>

                                <td>{{d[i].minExp}}</td>
                                <td>{{d[i].discountRate}}%</td>

                                <td>
								<@shiro.hasPermission name="users:usersGroup:edit">

                                    <button type="button" class="btn btn-primary btn-xs" onclick="edit('{{d[i].groupId}}')"> <i class="fa fa-paste"></i>编辑</button>
								</@shiro.hasPermission>
...
<div class="spiner-example">
	...
	function Ajaxpage(curr){
		$.ajax({
			url: "${basePath}/users/usersGroup/list",
	...
	function add(){
	    window.location.href="${basePath}/users/usersGroup/add";

	}
    function edit(id){
        window.location.href="${basePath}/users/usersGroup/edit/"+id;
	...
	function DelAll(){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                url: "${basePath}/users/usersGroup/delete",
                data:"id="+checkArray,
                type: "POST",
                dataType:'json',
